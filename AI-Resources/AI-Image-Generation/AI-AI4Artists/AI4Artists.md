...
> __bold__ entries signify my favorite resource(s) for that section/subsection (if I *HAD* to choose a single resource). Additionally each subsection is usually ordered by specificity of content (most general listed first).
...
* [DreamBooth: Fine Tuning Text-to-Image Diffusion Models for Subject-Driven Generation](https://dreambooth.github.io): Similar to Textual Inversion but instead focused on manipulating subject based images (i.e. *this thing/person/etc.* but *underwater*).
