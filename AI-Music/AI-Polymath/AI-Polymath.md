...
python polymath.py -a C:\Users\Ninja\Music\DEJA NORM'AL\Harmonic Songs/
...
```bash
python polymath.py -q songIDhere -t 120
```
...
```bash
python polymath.py -q songIDhere -k
```
...
```bash
python polymath.py -s songIDhere -sa 10
```

#### Search for similar songs based on a specific songs in the library and quantize all of them to tempo 120 BPM

```bash
python polymath.py -s songIDhere -sa 10 -q all -t 120
```

#### Include BPM as search criteria (-st)

```bash
python polymath.py -s songIDhere -sa 10 -q all -t 120 -st -k
```
...
```bash
python polymath.py -a songIDhere -q all -t 120 -m
```
